...
function LogIn() {
  ...
  const handleSignup = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(`${config.serverApi}/pos/store/signup`, {
  ...
  const verifyOtp = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(`${config.serverApi}/pos/store/verify-otp`, {
        email: formData.email,
        otp: otp,
      });
  ...
  <div className="relative w-full">
    <input type={showPassword ? "text" : "password"} name="password" placeholder="Password" className="input-style" onChange={handleChange} />
    <button type="button" className="absolute right-2 top-2" onClick={() => setShowPassword(!showPassword)}>
      {showPassword ? <EyeOffIcon className="h-10 w-5" /> : <EyeIcon className="h-10 w-5" />}
    </button>
    
  </div>
           {/* Confirm Password Input with Visibility Toggle */}
  <div className="relative w-full ">
    <input type={showConfirmPassword ? "text" : "password"} name="confirmPassword" placeholder="Confirm Password" className="input-style" onChange={handleChange} />
    <button type="button" className="absolute right-2 top-2" onClick={() => setShowConfirmPassword(!showConfirmPassword)}>
      {showConfirmPassword ? <EyeOffIcon className="h-10 w-5" /> : <EyeIcon className="h-10 w-5" />}
    </button>
  </div>
          {error && <p className="text-black text-sm">{error}</p>}
          <button className="btn-primary mt-4 w-full md:w-auto" onClick={handleSignup} disabled={loading}>
...
>
  ...
  {/* OTP Input Field
  <input type="text" name="otp" placeholder="Enter OTP" className="input-style mt-4" onChange={handleChange} /> */}
  ...
  {otpSent ? (
    <>
      ...
      />
      <button className="btn-primary w-full md:w-auto" onClick={verifyOtp} disabled={loading}>
        {loading ? "Verifying OTP..." : "Verify OTP"}
      </button>
    </>
  ) : (
    <button className="btn-primary w-full md:w-auto" onClick={handleLogin} disabled={loading}>
